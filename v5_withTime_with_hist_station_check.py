...
def run_simulation(case_df, fleet_state, travel_lookup):
    ...
    while i < total_cases:
        ...
        # 2) 額外記錄：historical_station 在這個時間點是否還有可用車
        hist_station = case_row["historical_station"]

        hist_station_available_units = fleet_state[
            (fleet_state["station_name"] == hist_station) &
            (fleet_state["status"] == "Available")
        ].copy()

        hist_station_available_count = len(hist_station_available_units)
        historical_station_has_available_unit = hist_station_available_count > 0

        # 3) 建立給選車函數用的案件資料（把 accepted_dt 換成 accepted_dt_sim）
        ...
        if dispatch_result is None:
            results.append({
                ...
                "historical_station": case_row["historical_station"],
                "historical_plate": case_row["historical_plate"],
                "historical_is_als": case_row["historical_is_als"],
                "historical_station_available_count": hist_station_available_count,
                "historical_station_has_available_unit": historical_station_has_available_unit,
                "accepted_dt": case_row["accepted_dt"],
                "grid_id": case_row["grid_id"],
                "assigned_ambulance": None,
                "assigned_station": None,
                "same_station_as_historical": pd.NA,
        ...
        results.append({
            ...
            "same_station_as_historical": dispatch_result["station_name"] == case_row["historical_station"],
