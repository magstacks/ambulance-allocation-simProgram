...
ONLY_SINGLE_DISPATCH = False   # 改成 False，因為現在要納入雙軌
...
SINGLE_DISPATCH_MODES = {
    "單一分隊救護",
}

DUAL_DISPATCH_MODES = {
    "雙軌出勤救護(僅限雙分隊出勤且皆到現場)",
    "雙軌出勤救護(雙分隊/救護車出勤且皆到現場)",
}

ALLOWED_DISPATCH_MODES = SINGLE_DISPATCH_MODES | DUAL_DISPATCH_MODES
...
def load_case_data():
    ...
    # 雙軌標記與需求欄位
    df["is_dual_dispatch"] = df["dispatch_mode"].isin(DUAL_DISPATCH_MODES)
    df["required_units"] = np.where(df["is_dual_dispatch"], 2, 1)
    df["require_als"] = np.where(df["is_dual_dispatch"], 1, 0)

    # historical_is_als 轉成布林，避免排序和配對出問題
    df["historical_is_als"] = (
        df["historical_is_als"]
        .replace({
    ...
    # 現在保留：單一分隊救護 + 指定兩種雙軌
    df = df[df["dispatch_mode"].isin(ALLOWED_DISPATCH_MODES)].copy()
    ...
    df = df.sort_values(
        ["accepted_dt_sim", "case_id", "is_dual_dispatch", "historical_is_als"],
...
# Helper Function
def collect_dispatch_group(case_df, start_idx):
    """
    若是單軌，回傳 1 列
    若是雙軌，回傳同案號、同 accepted_dt_sim、且屬於雙軌模式的整組列
    """
    first_row = case_df.iloc[start_idx]

    if not bool(first_row["is_dual_dispatch"]):
        return case_df.iloc[[start_idx]].copy().reset_index(drop=True), start_idx + 1

    case_id = first_row["case_id"]
    accepted_dt_sim = first_row["accepted_dt_sim"]
    ...
    while j < len(case_df):
        row = case_df.iloc[j]
        same_group = (
            bool(row["is_dual_dispatch"])
            and row["case_id"] == case_id
            and row["accepted_dt_sim"] == accepted_dt_sim
        )
        if not same_group:
            break
    ...
    group_df = pd.DataFrame(rows).reset_index(drop=True)
    return group_df, j
...
def run_simulation(case_df, fleet_state, travel_lookup):
    ...
    total_rows = len(case_df)
    start_clock = time.time()

    i = 0
    while i < total_rows:
        group_df, next_i = collect_dispatch_group(case_df, i)

        case_row = group_df.iloc[0]
        current_time = case_row["accepted_dt_sim"]
        is_dual = bool(case_row["is_dual_dispatch"])

        if i == 0 or ((i + 1) % PROGRESS_EVERY == 0):
            elapsed_min = (time.time() - start_clock) / 60
            print(
                f"[Progress] row {i + 1:,}/{total_rows:,} "
                f"({(i + 1) / total_rows:.1%}) | "
        ...
        case_row_for_sim = case_row.copy()
        case_row_for_sim["accepted_dt"] = case_row["accepted_dt_sim"]
        case_row_for_sim["required_units"] = int(group_df["required_units"].max())
        case_row_for_sim["require_als"] = int(group_df["require_als"].max())

        dual_dispatch_result = select_dual_dispatch_units(
            case_row=case_row_for_sim,
            fleet_state=fleet_state,
            travel_lookup=travel_lookup,
        )

        if dual_dispatch_result is None:
            for _, hist_row in group_df.iterrows():
                results.append({
                    "case_id": hist_row["case_id"],
                    "case_status": hist_row["case_status"],
                    "dispatch_mode": hist_row["dispatch_mode"],
                    "handling_type": hist_row["handling_type"],
                    "task_scenario": hist_row["task_scenario"],
                    "is_dual_dispatch": True,
                    "accepted_dt": hist_row["accepted_dt"],
                    "accepted_dt_raw": hist_row["accepted_dt_raw"],
                    "accepted_dt_sim": hist_row["accepted_dt_sim"],
                    "accepted_dt_fixed_from_dispatch": hist_row["accepted_dt_fixed_from_dispatch"],
                    "grid_id": hist_row["grid_id"],
                    "assigned_ambulance": None,
                    "assigned_station": None,
                    "assigned_is_als": np.nan,
                    "dispatch_dt_sim": pd.NaT,
                    ...
                    "response_min_sim": np.nan,
                    "mission_end_dt_sim": pd.NaT,
                    "historical_response_min": hist_row["historical_response_min"],
                    "status": "NO_UNIT_OR_NO_TRAVEL_TIME_DUAL",
                })
            i = next_i
            continue

        selected_units = dual_dispatch_result["selected_units"]

        # 歷史列：ALS 列優先
        group_hist_sorted = group_df.sort_values(
            ["historical_is_als", "service_minutes_for_sim"],
            ascending=[False, False]
        ).reset_index(drop=True)

        # 模擬選到的車：ALS 車優先
        selected_units_sorted = sorted(
            selected_units,
            key=lambda x: (not bool(x["is_als"]), x["travel_min_sim"])
        )

        n_pair = min(len(group_hist_sorted), len(selected_units_sorted))

        for k in range(n_pair):
            hist_row = group_hist_sorted.iloc[k]
            unit = selected_units_sorted[k]

            if hist_row["task_scenario"] == "NO_SCENE":
                mission_end_dt = unit["dispatch_dt_sim"] + pd.Timedelta(
                    minutes=float(hist_row["service_minutes_for_sim"])
                )
                arrival_dt_out = pd.NaT
                response_min_out = np.nan
                status_out = "OK_DUAL_NO_SCENE"
            else:
                mission_end_dt = unit["arrival_dt_sim"] + pd.Timedelta(
                    minutes=float(hist_row["service_minutes_for_sim"])
                )
                arrival_dt_out = unit["arrival_dt_sim"]
                response_min_out = unit["response_min_sim"]
                status_out = "OK_DUAL"
            ...
            results.append({
                ...
                "accepted_dt_fixed_from_dispatch": hist_row["accepted_dt_fixed_from_dispatch"],
                "grid_id": hist_row["grid_id"],
                "assigned_ambulance": unit["ambulance_id"],
                "assigned_station": unit["station_name"],
                "assigned_is_als": unit["is_als"],
                "dispatch_dt_sim": unit["dispatch_dt_sim"],
                "arrival_dt_sim": arrival_dt_out,
                "travel_min_sim": unit["travel_min_sim"],
                "wait_min_sim": unit["wait_min_sim"],
                "response_min_sim": response_min_out,
                "mission_end_dt_sim": mission_end_dt,
                "historical_response_min": hist_row["historical_response_min"],
                "status": status_out,
    ...
    print(
        f"[Progress] done: {total_rows:,}/{total_rows:,} "
...
def summarize_result(result_df: pd.DataFrame):
    ok_df = result_df[result_df["status"].str.startswith("OK", na=False)].copy()
    ...
    return pd.DataFrame([{
        ...
        "n_ok_dual": int((ok_df["is_dual_dispatch"] == True).sum()) if len(ok_df) > 0 else 0,
